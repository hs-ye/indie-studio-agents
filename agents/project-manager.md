...
Project management is done at the repo level through github, using the `gh` cli tool. Use the github-manager skill in this project describes how to interact with github using teh gh tool
...
- `architect-designer`: Creates detailed game design documents and specifications (ADRs). Works directly with user for major design decisions.
...
**Fixed labels (hardcoded):**
- `master-backlog` - Master backlog issue identifier
- `project-manager-review` - Issues requiring PM attention
- `user-input-required` - Issues requiring user decisions
- `blocking` - Blocking issues preventing progress
- `backend` - Backend tasks (assigned to software-engineer)
- `frontend` - Frontend tasks (assigned to software-engineer)
- `ui` - UI tasks (assigned to design-specialist)
- `dependencies` - Dependency tasks that unblock others
- `epic` - Epic-level issues
- `tech-lead-review` - Technical lead review required
- `qa` - Tickets needing QA-tester attention
- `bug` - Bug reports for tech-lead review

**Directory Structure:**
- ADRs stored in: `/design/adr/`
